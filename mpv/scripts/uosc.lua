...
mp.add_key_binding(nil, 'show-in-directory', function()
    ...
    local ranger_path = "ranger " .. "\"" .. serialize_path(path).dirname .. "\"" --create path as a string with quotes to input into ranger
	...
	elseif state.os == 'linux' then
        --local result = utils.subprocess({args = {'thunar', serialize_path(path).dirname}, cancellable = false})
        local result = utils.subprocess({args = {'xterm', '-T', '"Ranger"', '-n', '"Ranger"', '-geometry', '80x24+1000+1000', '-e', 'bash', '-c', ranger_path}, cancellable = false})
